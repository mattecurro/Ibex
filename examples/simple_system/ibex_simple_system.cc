...
// Get the current time
  ...
  // Print the string
  std::cout << "Current date and time: " << now_str << std::endl;


#define REAL_DATA_SIZE_STR 210000
#define TYPOLOGY "small"

  int64_t blksize = REAL_DATA_SIZE_STR/3;
  int64_t i=0,j=0, N=0;
	//blksize = blksize*20;
	while (j<blksize) {
		i++;
...
//	N=i-1+10;
  ...
  std::string typologyStr = TYPOLOGY;
  std::cout << "str:" << typologyStr << std::endl;
  std::string realDataSizeStr = std::to_string(REAL_DATA_SIZE_STR);
  std::cout << "RealdataSizeStr:" << realDataSizeStr << std::endl;

  std::ofstream pcount_csv1("ibex_simple_system_pcount.csv", std::ios_base::app);

  std::ofstream pcount_csv2("ibex_simple_system_pcount" + realDataSizeStr + ".csv");
  pcount_csv1 << ibex_pcount_string(true);
  pcount_csv2 << ibex_pcount_string(true);

  std::ofstream pcount_csv3("ibex_simple_system_pcount.csv", std::ios_base::app);
  std::ofstream pcount_csv4("ibex_simple_system_pcount" + realDataSizeStr + ".csv", std::ios_base::app);

  pcount_csv3 << "Configuration," << typologyStr << std::endl;
  // Append the value of N to the file, followed by a comma
  pcount_csv3 << "Matrix Dimension," << N << std::endl;
  pcount_csv4 << "Matrix Dimension," << N << std::endl;
  return true;
}
