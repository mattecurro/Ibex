...
// aggiunta da me &(results[i].mat) 
//						Dichiarazione funzione:		 ee_u16 core_bench_matrix(mat_params *p, ee_s16 seed, ee_u16 crc);
	for (i=0; i<iterations; i++) {
		crc=core_bench_matrix(&(res->mat),res->seed1, res->crc);
		res->crc=crcu16(crc,res->crc);
		if (i==0) res->crcmatrix=res->crc;
	}
...
/*		if (results[i].execs & ID_LIST) {
		...
		if (results[i].execs & ID_MATRIX) {
			ee_printf("total_real_data_size %d	\n", results[0].size);
